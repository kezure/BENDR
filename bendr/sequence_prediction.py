...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        type=int,
        help="Sequence regression starting point.",
    )
    parser.add_argument(
        "--num-sequence", default=None, type=int, help="Number of regression points."
    )
...
def run(dataset, ds_name, args, pre_exp, seq_exp, result_tracker) -> None:
    ...
    process = BendingCollegeWav2Vec(
        encoder, contextualizer, **pre_exp.bending_college_args
    )
    result_tracker.add_results_all_thinkers(
        process, ds_name, dataset, sequence_length=dataset.sequence_length
    )
...
if __name__ == "__main__":
    ...
    for ds_name, ds in tqdm.tqdm(
        sequence_experiments.datasets.items(),
        total=len(sequence_experiments.datasets.items()),
        desc="Datasets",
    ):
        ...
        if args.min_sequence is not None and args.num_sequence is not None:
            ...
            logspace = list(
                reversed(
                    np.logspace(
                        np.log10(args.min_sequence),
                        np.log10(sequence_experiments.global_samples),
                        num=args.num_sequence,
                    ).astype(int)
                )
            )
