...
CHANCE_DB = {"MMI": 0.5, "BCIC": 0.25, "ERN": 0.5, "P300": 0.5, "SSC": 0.2}
...
def downstream_plot_performance(df):
    ...
    ax.legend(
        [
            handles[labels.index(l) + len(LEGEND_ORDER)]
            for i, l in enumerate(LEGEND_ORDER)
        ],
        [f"({i+1}.) {l}" for i, l in enumerate(LEGEND_ORDER)],
        title="Models",
        handletextpad=0,
        columnspacing=1,
        fontsize="x-small",
...
def sequence_plot_performance(df, n_boot=None):
    ...
    sns.violinplot(
        x="Dataset", y="Accuracy", data=df, palette="pastel", inner="quartiles"
    )
...
def regression_plot(df):
    ...
    sns.lineplot(
        data=df, x="sequence_length", y="Accuracy", hue="Dataset", palette="pastel"
    )
    plt.xscale("log")
...
def xlsx_to_df(spreadsheet):
    df = pd.concat(
        pd.read_excel(spreadsheet, sheet_name=None, engine="openpyxl").values(),
        ignore_index=True,
    )
...
def downstream_plot(args):
    ...
    for ds in METRICS_DB:
        df.loc[df["Dataset"] == ds, "metric"] = (
            df[df["Dataset"] == ds][METRICS_DB[ds]] - CHANCE_DB[ds]
        ) / (1 - CHANCE_DB[ds])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1000,
        type=int,
        help="Number of bootstrap iterations to perform when "
    ...
    downstream_parser = subparsers.add_parser(
        "downstream", help="Plot all the downstream results from a directory."
    )
    downstream_parser.add_argument(
        "directory", help="Directory containing '.xlsx' files with performance results."
    )
    downstream_parser.set_defaults(func=downstream_plot)

    sequence_parser = subparsers.add_parser(
        "sequences", help="Plot the sequence likelihoods."
    )
    sequence_parser.add_argument(
        "--filename",
        default="seq_results.xlsx",
        help="The name of the sequence results " "file.",
    )
    sequence_parser.set_defaults(func=sequence_likelihood_plot)

    regression_parser = subparsers.add_parser(
        "regression", help="Plot the sequence likelihoods."
    )
    regression_parser.add_argument(
        "--filename",
        default="seq-regression.xlsx",
        help="The name of the sequence results " "file.",
    )
