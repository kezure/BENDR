...
def load_datasets(experiment):
    ...
    for name, ds in experiment.datasets.items():
        ...
        if (
            hasattr(experiment, "validation_dataset")
            and experiment.validation_dataset == name
        ):
            validation = dataset
            continue
    ...
    print(
        f"Training BENDR using {total_thinkers} people's data across {len(training)} datasets."
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        default="configs/pretraining.yml",
        help="The DN3 config file to use.",
    )
    parser.add_argument(
        "--hidden-size", default=512, type=int, help="The hidden size of the encoder."
    )
    parser.add_argument(
        "--resume",
        default=None,
        type=int,
        help="Whether to continue training the encoder from the " "specified epoch.",
    )
    parser.add_argument("--num-workers", default=6, type=int)
    parser.add_argument(
        "--no-save", action="store_true", help="Don't save checkpoints while training."
    )
    parser.add_argument(
        "--no-save-epochs",
        action="store_true",
        help="Don't save epoch checkpoints while training",
    )
...
if __name__ == "__main__":
    ...
    tqdm.tqdm.write(
        encoder.description(experiment.global_sfreq, experiment.global_samples)
    )
    contextualizer = BENDRContextualizer(
        encoder.encoder_h, layer_drop=experiment.bending_college_args.layer_drop
    )
    if args.resume is not None:
        encoder.load(f"checkpoints/encoder_epoch_{args.resume}.0.pt")
        contextualizer.load(f"checkpoints/contextualizer_epoch_{args.resume}.0.pt")

    process = BendingCollegeWav2Vec(
        encoder, contextualizer, **experiment.bending_college_args
    )
