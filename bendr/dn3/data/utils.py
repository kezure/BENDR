    ...
    def __init__(
        ...
        *datasets,
        ...
        if oversample:
            max_artificial_size = (
                max(dataset_sizes)
                if max_artificial_size is None
                else max_artificial_size
            )
            largest_oversample = min(max(dataset_sizes), max_artificial_size)
            sample_sizes = [
                largest_oversample if s < largest_oversample else s
        ...
        for ds_len, inflated_size in zip(dataset_sizes, sample_sizes, strict=False):
            ind_to_ind = np.tile(
                np.arange(ds_len), int(np.ceil(inflated_size / ds_len))
            )
...
class ThinkerSubset(Thinker):
    ...
    def split_all_trials(session, frac=0.5, shuffled=True):
        ...
        return ThinkerSubset(session, inds[: int(frac * len(inds))]), ThinkerSubset(
            session, inds[int(frac * len(inds)) :]
        )

    def __init__(self, thinker: Thinker, split_inds):
        self.thinker = thinker
        self.inds = dict(zip(range(len(split_inds)), split_inds, strict=False))
        self.__dict__.update(
            {
                k: v
                for k, v in thinker.__dict__.items()
                if k not in ["__len__", "__getitem__", "get_targets", "_transforms"]
            }
        )
...
def get_dataset_max_and_min(dataset: Dataset, rd=0.9):
    ...
    for data in pbar:
        ...
        pbar.set_postfix(
            dict(dmax=dmax, dmin=dmin, dev_max=running_dev_max, dev_min=running_dev_min)
        )
...
class SingleStatisticSpanRejection:
    ...
    def reset(self, rejections_only=False):
        self.rejections = {
            thid: {sid: [] for sid in self.dataset.thinkers[thid].sessions.keys()}
            for thid in self.dataset.get_thinkers()
        }
        self.exclude = dict()
        ...
        self._sfreq, self._sequence_length = (
            self.dataset.sfreq,
            self.dataset.sequence_length,
        )
        self.old_trial, self.old_session, self.old_person = (
            self.dataset.return_trial_id,
    ...
    def valid_stats(self):
        ...
        for tid in tqdm.tqdm(
            self.statistic_lookup, desc="Collecting...", unit="Person"
        ):
    ...
    def collect_statistic(self):
        ...
        session_lists = {
            who_dis: list(self.dataset.thinkers[who_dis].sessions.keys())
            for who_dis in thinker_list
        }
        ...
        for data in tqdm.tqdm(batch_loader, desc="Loading Dataset"):
            ...
            for i, (t, s) in enumerate(zip(think_id, sess_id, strict=False)):
                who_dis = thinker_list[int(t)]
                which_sess = session_lists[who_dis][s]
                self.statistic_lookup[who_dis][which_sess].append(x[i])

        self.dataset.update_id_returns(
            trial=self.old_trial, person=self.old_person, session=self.old_session
        )

    def _make_numpy_convenience(self):
        stats = {
            thid: {sid: [] for sid in self.statistic_lookup[thid].keys()}
            for thid in self.statistic_lookup
        }
        for tid in stats:
    ...
    def deviation_threshold_rejection(self, reject_iterations=10, num_deviations=4):
        ...
        for i in tqdm.trange(reject_iterations, desc="Rejecting:"):
            ...
            for tid in stats:
                for sid in stats[tid]:
                    normed_diffs = (
                        np.abs(stats[tid][sid] - total_mean_dev) / total_stdev_of_dev
                    )

                    new_rejections = np.unique(
                        np.ma.nonzero(normed_diffs >= num_deviations)[0]
                    )
                    stats[tid][sid][new_rejections] = np.ma.masked
                    reject_count += len(new_rejections)
                    num_rejections += len(new_rejections)
                    self.rejections[tid][sid] = np.union1d(
                        new_rejections, self.rejections[tid][sid]
                    ).tolist()
    ...
    def keep_window(self, low=None, high=None):
        ...
        for tid in stats:
            for sid in stats[tid]:
                ...
                num_rejections += len(new_rejections)
                stats[tid][sid][new_rejections] = np.ma.masked
                self.rejections[tid][sid] = np.setdiff1d(
                    new_rejections, self.rejections[tid][sid]
                ).tolist()
    ...
    def get_configuratron_exclusions(self, save_to_file=None):
        ...
        for tid in tqdm.tqdm(
            self.statistic_lookup, desc="Determining bad spans...", unit="person"
        ):
            for sid in self.statistic_lookup[tid]:
