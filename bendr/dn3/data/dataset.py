...
from dn3.utils import DN3atasetException, DN3atasetNanFound, rand_split, unfurl
                ...
                for i in range(len(x)):
                    if torch.any(torch.isnan(x[i])):
                        raise DN3atasetNanFound(
                            f"NaN generated by transform {self} for {i}'th tensor"
                        )
        return x
        ...
        **dataloader_kwargs,
        ...
        for batch in tqdm.tqdm(loader, desc="Loading Batches"):
            ...
            if loaded is None:
                ...
                loaded = [
                    np.concatenate([loaded[i], batch[i]], axis=0)
...
class _Recording(DN3ataset, ABC):
    ...
    def __init__(self, info, session_id, person_id, tlen, ch_ind_picks=None):
        ...
        self.info = info
        self.picks = (
            ch_ind_picks if ch_ind_picks is not None else list(range(len(info["chs"])))
        )
        self._recording_channels = [
            (ch["ch_name"], int(ch["kind"]))
    ...
    def get_all(self):
        ...
        return [torch.stack(t) for t in zip(*all_recordings, strict=False)]
        ...
        **kwargs,
        ...
        for start, stop in bad_spans:
            ...
            for i, span_start in enumerate(self._decimated_sequence_starts):
                if (
                    start <= span_start < stop
                    or start <= span_start + self._recording_len <= stop
                ):
        ...
        if self._stride_load and len(self._decimated_sequence_starts) > 0:
            ...
            x = x[:, ::decimate]
            self._x = np.empty(
                [x.shape[0], self._recording_len, len(self._decimated_sequence_starts)],
                dtype=x.dtype,
            )
    ...
    def __getitem__(self, index):
        ...
        if self._stride_load:
            ...
            x = self.raw.get_data(
                self.picks,
                start=start,
                stop=start + self._recording_len * self.decimate,
            )
            if self.decimate > 1:
                x = x[:, :: self.decimate]
        ...
        if torch.any(torch.isnan(x)):
            print(f"Nan found: raw {self.filename}, index {index}")
...
class EpochTorchRecording(_Recording):
    def __init__(
        ...
        super().__init__(
            ...
            session_id,
            person_id,
            epochs.tmax - epochs.tmin + 1 / epochs.info["sfreq"],
            ch_ind_picks,
        )
        self.epochs = epochs
        ...
        if force_label:
            ...
            self.epoch_codes_to_class_labels = {
                v: i for i, v in enumerate(sorted(reverse_mapping.keys()))
            }
        skip_epochs = list() if skip_epochs is None else skip_epochs
        self._skip_map = [
            i for i in range(len(self.epochs.events)) if i not in skip_epochs
        ]
        self._skip_map = dict(
            zip(range(len(self._skip_map)), self._skip_map, strict=False)
        )
    ...
    def __getitem__(self, index):
        ...
        if self._cache is None or self._cache[index] is None:
            ...
            if len(x.shape) != 3 or 0 in x.shape:
                print(
                    f"I don't know why: {self.epochs.filename} index{index}/{len(self)}"
                )
                print(self.epochs.info["description"])
                print(f"Using trial {index-1} in place for now...")
        ...
        y = (
            torch.tensor(self.epoch_codes_to_class_labels[ep.events[0, -1]])
            .squeeze()
            .long()
        )
    ...
    def get_targets(self):
        return np.apply_along_axis(
            lambda x: self.epoch_codes_to_class_labels[x[0]],
            1,
            self.epochs.events[list(self._skip_map.values()), -1, np.newaxis],
        ).squeeze()
...
class Thinker(DN3ataset, ConcatDataset):
    ...
    def __str__(self):
        return f"Person {self.person_id} - {len(self)} trials | {len(self._transforms)} transforms"
    ...
    def __getitem__(self, item, return_id=False):
        ...
        if self.return_trial_id:
            trial_id = (
                item
                if session_idx == 0
                else item - self.cumulative_sizes[session_idx - 1]
            )
    ...
    def split(
        self,
        training_sess_ids=None,
        validation_sess_ids=None,
        testing_sess_ids=None,
        test_frac=0.25,
        ...
        """
        training_sess_ids = (
            set(training_sess_ids) if training_sess_ids is not None else set()
        )
        validation_sess_ids = (
            set(validation_sess_ids) if validation_sess_ids is not None else set()
        )
        testing_sess_ids = (
            set(testing_sess_ids) if testing_sess_ids is not None else set()
        )
        duplicated_ids = training_sess_ids.intersection(
            validation_sess_ids
        ).intersection(testing_sess_ids)
        if len(duplicated_ids) > 0:
            print(f"Ids duplicated across train/val/test split: {duplicated_ids}")
        use_sessions = self.sessions.copy()
        training, validating, testing = (
            self._make_like_me({s_id: use_sessions.pop(s_id) for s_id in ids})
            if len(ids)
            else None
        ...
        if training is not None and validating is not None and testing is not None:
            if len(use_sessions) > 0:
                print(
                    f"Warning: sessions specified do not span all sessions. Skipping {len(use_sessions)} sessions."
                )
    ...
    def preprocess(
        self, preprocessor: Preprocessor, apply_transform=True, sessions=None, **kwargs
    ):
        """
        Applies a preprocessor to the dataset
        ...
        for sid, session in enumerate(self.sessions.values()):
            session.preprocess(
                preprocessor, session_id=sid, apply_transform=apply_transform, **kwargs
            )
        return preprocessor
...
class DatasetInfo:
    ...
    def __str__(self):
        return f"{self.dataset_name} | {self.targets} targets | Excluding {self.excluded_people}"
...
class Dataset(DN3ataset, ConcatDataset):
    ...
    def __init__(
        ...
        if not isinstance(thinkers, Iterable):
            raise ValueError(
                "Provided thinkers must be in an iterable container, e.g. list, tuple, dicts"
            )
        ...
        for t in natural_sort(thinkers.keys()):
            self.__add__(
                ...
                return_trial_id=return_trial_id,
            )
        self._reset_dataset()

        self.dataset_id = (
            torch.tensor(dataset_id).long() if dataset_id is not None else None
        )
        self.task_id = torch.tensor(task_id).long() if task_id is not None else None
        self.update_id_returns(
            return_trial_id,
            return_session_id,
            return_person_id,
            return_dataset_id,
    ...
    def update_id_returns(
        self, trial=None, session=None, person=None, task=None, dataset=None
    ):
        """
    ...
    def __str__(self):
        ds_name = (
            ...
            else self.info.dataset_name
        )
        return f">> {ds_name} | DSID: {self.dataset_id} | {len(self.get_thinkers())} people | {len(self)} trials | {len(self.channels)} channels | {self.sequence_length} samples/trial | {self.sfreq}Hz | {len(self._transforms)} transforms"
    ...
    def __add__(
        ...
        return_session_id = (
            self.return_session_id if return_session_id is None else return_session_id
        )
        return_trial_id = (
            self.return_trial_id if return_trial_id is None else return_trial_id
        )
        ...
        if thinker.person_id in self.thinkers.keys():
            print(
                f"Warning. Person {thinker.person_id} already in dataset... Merging sessions."
            )
    ...
    def __getitem__(self, item):
        ...
        if self._safe_mode:
            for i in range(len(x)):
                if torch.any(torch.isnan(x[i])):
                    raise DN3atasetNanFound(
                        f"Nan found at tensor offset {i}. "
                        f"Loading data from person {person} and sample {sample_idx}"
                    )
        ...
        if self._safe_mode:
            try:
                ...
                raise DN3atasetNanFound(
                    f"Nan found after transform | {e.args} | from person {person} and sample {sample_idx}"
                )
    ...
    def preprocess(
        self, preprocessor: Preprocessor, apply_transform=True, thinkers=None
    ):
        """
        Applies a preprocessor to the dataset
        ...
        for thid, thinker in enumerate(self.thinkers.values()):
            thinker.preprocess(
                preprocessor, thinker_id=thid, apply_transform=apply_transform
            )
        return preprocessor
    ...
    def channels(self):
        ...
        if not same_channel_sets(channels):
            warnings.warn(
                f"Multiple channel sets found {[len(c) for c in channels]}. "
    ...
    def _make_like_me(self, people: list):
        if len(people) == 1:
            ...
            like_me = Dataset(
                {p: self.thinkers[p] for p in people},
                dataset_id,
                task_id,
                return_person_id=self.return_person_id,
                return_session_id=self.return_session_id,
                return_dataset_id=self.return_dataset_id,
                return_task_id=self.return_task_id,
                return_trial_id=self.return_trial_id,
    ...
    def _generate_splits(self, validation, testing):
        for val, test in zip(validation, testing, strict=False):
            ...
            _val_set = (
                set(validating.get_thinkers())
                if len(val) > 1
                else {validating.person_id}
            )

            testing = self._make_like_me(test)
            _test_set = (
                set(testing.get_thinkers()) if len(test) > 1 else {testing.person_id}
            )

            if len(_val_set.intersection(_test_set)) > 0:
                raise ValueError(
                    f"Validation and test overlap with ids: {_val_set.intersection(_test_set)}"
                )

            print(f"Training:   {training}")
            print(f"Validation: {validating}")
    ...
    def loso(self, validation_person_id=None, test_person_id=None):
        ...
        test : Thinker
               The test thinker
        """
        if isinstance(test_person_id, (str, int)) and isinstance(
            validation_person_id, (str, int)
        ):
            yield from self._generate_splits(
                [[validation_person_id]], [[test_person_id]]
            )
            return
        elif isinstance(test_person_id, str):
            yield from self._generate_splits(
                [[v] for v in self.get_thinkers() if v != test_person_id],
        ...
        if test_person_id is None and isinstance(validation_person_id, (str, int)):
            test_person_id = [
                t for t in self.get_thinkers() if t != validation_person_id
            ]
            validation_person_id = [
                validation_person_id for _ in range(len(test_person_id))
            ]
        ...
        if validation_person_id is None:
            validation_person_id = [
                test_person_id[i - 1] for i in range(len(test_person_id))
            ]

        if not isinstance(test_person_id, list) or len(test_person_id) != len(
            validation_person_id
        ):
            raise ValueError("Test ids must be same length iterable as validation ids.")

        yield from self._generate_splits(
            [[v] for v in validation_person_id], [[t] for t in test_person_id]
        )
    ...
    def lmso(self, folds=10, test_splits=None, validation_splits=None):
        ...
        def is_nested(split: list):
            ...
            for x in split[1:]:
                if (should_be_nested and not isinstance(x, (list, tuple))) or (
                    isinstance(x, (list, tuple)) and not should_be_nested
                ):
                    raise ValueError(
                        "Can't mix list/tuple and other elements when specifying ids."
                    )
            if not should_be_nested and folds is None:
                raise ValueError(
                    "Can't infer folds from non-nested list. Specify folds, or nest ids"
                )
        ...
        def calculate_from_remainder(known_split):
            ...
            if folds is None:
                print(f"Inferred {_folds} folds from test split.")
            remainder = list(set(self.get_thinkers()).difference(known_split))
            return [list(x) for x in np.array_split(remainder, _folds)], [
                known_split for _ in range(_folds)
            ]
        ...
        if test_splits is None and validation_splits is None:
            ...
            test_splits, validation_splits = zip(
                *[(folds[i], folds[i - 1]) for i in range(len(folds))], strict=False
            )
    ...
    def dump_dataset(
        ...
        for i in tqdm.trange(round(len(self) / chunksize), desc="Saving", unit="files"):
            ...
            accumulated = [torch.stack(z) for z in zip(*accumulated, strict=False)]
            if compressed:
...
class DumpedDataset(DN3ataset):
    def __init__(
        ...
        self.filenames = sorted(
            [f for f in self.toplevel.iterdir() if f.name != summary_file.name],
            key=lambda f: int(f.stem),
        )

        self.cache = (
            np.empty((self._len, len(self.channels), self.sequence_length))
            if cache_all
            else None
        )
    ...
    def __str__(self):
        ...
        return ">> {} | {} people | {} trials | {} channels | {} samples/trial | {}Hz | {} transforms | ".format(
            "ds_name",
            len(self.thinkers),
            len(self),
            len(self.channels),
    ...
    def preprocess(self, preprocessor: Preprocessor, apply_transform=True):
        raise DN3atasetException(
            "Can't preprocess dumped dataset. Load from original files to do this."
        )

    def __getitem__(self, index):
        if self.aux_cache is not None and self.aux_cache[index] is not None:
            print(
                f"Hit: chunk {index // self._chunksize}, id: {id(self.cache[index // self._chunksize])}"
            )
            ...
            if self.aux_cache is not None and self.aux_cache[index] is None:
                ...
                for i in [offset]:
                    ...
                    self.aux_cache[index] = [
                        torch.from_numpy(data[f])[i] for f in data.files[1:]
                    ]
                data = [self.cache[index], *self.aux_cache[index]]
            else:
