...
class DN3BaseModel(nn.Module):  # type: ignore[misc]
    ...
    def from_dataset(cls, dataset: DN3ataset, **modelargs):
        ...
        return cls(
            samples=dataset.sequence_length, channels=len(dataset.channels), **modelargs
        )
...
class Classifier(DN3BaseModel):
    ...
    def from_dataset(cls, dataset: DN3ataset, **modelargs):
        ...
        print(
            "Creating {} using: {} channels x {} samples at {}Hz | {} targets".format(
                cls.__name__,
    ...
    def __init__(self, targets, samples, channels, return_features=True):
        super(Classifier, self).__init__(
            samples, channels, return_features=return_features
        )
...
class StrideClassifier(Classifier, metaclass=ABCMeta):
    def __init__(
        ...
        super(StrideClassifier, self).__init__(
            targets, samples, channels, return_features=return_features
        )

    def make_new_classification_layer(self):
        self.classifier = torch.nn.Conv1d(
            self.num_features_for_classification,
            self.targets,
...
class EEGNet(Classifier):
    ...
    def __init__(
        ...
        if t_len >= samples:
            print(
                "Warning: EEGNet `t_len` too long for sample length, reverting to 0.25 sample length"
            )
            t_len = samples // 4
        ...
        self.sep_conv = nn.Sequential(
            ...
            nn.Conv2d(
                D * F1, D * F1, (1, 17), bias=False, padding=(0, 8), groups=D * F1
            ),
...
class BENDRClassifier(Classifier):
    def __init__(
        ...
        **kwargs,
        ...
        super(BENDRClassifier, self).__init__(
            targets, samples, channels, return_features=return_features
        )
