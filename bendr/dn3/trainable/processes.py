...
from dn3.data.ood import fracture_dataset_into_thinker_domains
...
class BaseProcess:
        ...
        **kwargs,
        ...
        for member in new_members:
            if isinstance(self.__dict__[member], (torch.nn.Module, torch.Tensor)):
                if not (
                    isinstance(self.__dict__[member], torch.Tensor)
                    and not self.__dict__[member].requires_grad
                ):
    ...
    def _optimize_dataloader_kwargs(self, num_worker_cap=6, **loader_kwargs):
        ...
        print(
            "Loading data with {} additional workers".format(
    ...
    def _get_batch(self, iterator):
        batch = [
            x.to(self.device, non_blocking=self.cuda == "cuda") for x in next(iterator)
        ]
    ...
    def predict(self, dataset, **loader_kwargs):
        ...
        def package_multiple_tensors(batches: list):
            if isinstance(batches[0], torch.Tensor):
                ...
                return [torch.cat(b) for b in zip(*batches, strict=False)]
    ...
    def standard_logging(cls, metrics: dict, start_message="End of Epoch"):
        ...
        for m in metrics:
            if "acc" in m.lower() or "pct" in m.lower():
                start_message += f" {m}: {metrics[m]:.2%} |"
            elif m == "lr":
                start_message += f" {m}: {metrics[m]:.3e} |"
            else:
                start_message += f" {m}: {metrics[m]:.3f} |"
    ...
    def save_best(self):
        ...
        """
        return [
            {k: v.cpu() for k, v in self.__dict__[m].state_dict().items()}
            for m in self._trainables
        ]
    ...
    def load_best(self, best):
        ...
        best: Any
        """
        for m, state_dict in zip(self._trainables, best, strict=False):
            self.__dict__[m].load_state_dict(
    ...
    def _retain_best(self, old_checkpoint, metrics_to_check: dict, retain_string: str):
        ...
        def found_best():
            tqdm.tqdm.write(f"Best {retain_string}. Retaining checkpoint...")
        ...
        if self.best_metric is None:
            ...
            retain_string == "loss"
            and metrics_to_check[retain_string] <= self.best_metric
        ):
            best_checkpoint = found_best()
        elif (
            retain_string != "loss"
    ...
    def _make_dataloader(self, dataset, training=False, **loader_kwargs):
        ...
        return DataLoader(
            dataset, **self._dataloader_args(dataset, training, **loader_kwargs)
        )
    ...
    def fit(
        ...
        **loader_kwargs,
        ...
        training_dataset = self._make_dataloader(
            training_dataset, training=True, **loader_kwargs
        )
        ...
        resume_iteration = (
            1 if resume_iteration is None else resume_iteration % len(training_dataset)
        )

        _clear_scheduler_after = self.scheduler is None
        if _clear_scheduler_after:
            last_epoch_workaround = (
                len(training_dataset) * (resume_epoch - 1) + resume_iteration
            )
            last_epoch_workaround = (
                -1 if last_epoch_workaround <= 1 else last_epoch_workaround
            )
            self.set_scheduler(
        ...
        train_log_interval = (
            len(training_dataset) if train_log_interval is None else train_log_interval
        )
        metrics = OrderedDict()
        ...
        def update_metrics(new_metrics: dict, iterations):
            if len(metrics) == 0:
                ...
                for m in new_metrics:
                    try:
                        metrics[m] = (
                            metrics[m] * (iterations - 1) + new_metrics[m]
                        ) / iterations
                    except KeyError:
        ...
        def print_training_metrics(epoch, iteration=None):
            if iteration is not None:
                self.standard_logging(
                    metrics, f"Training: Epoch {epoch} - Iteration {iteration}"
                )
            else:
                self.standard_logging(metrics, f"Training: End of Epoch {epoch}")

        def _validation(epoch, iteration=None):
            _metrics = self.evaluate(validation_dataset, **loader_kwargs)
            if iteration is not None:
                self.standard_logging(
                    _metrics, f"Validation: Epoch {epoch} - Iteration {iteration}"
                )
            else:
                self.standard_logging(_metrics, f"Validation: End of Epoch {epoch}")
        ...
        for epoch in epoch_bar:
            ...
            for iteration in pbar:
                ...
                if "momentum" in self.optimizer.defaults:
                    train_metrics["momentum"] = self.optimizer.param_groups[0][
                        "momentum"
                    ]
                ...
                if (
                    isinstance(validation_interval, int)
                    and (iteration % validation_interval == 0)
...
class StandardClassification(BaseProcess):
    def __init__(
        ...
        **kwargs,
        ...
        super(StandardClassification, self).__init__(
            cuda=cuda,
            lr=learning_rate,
        ...
        if (
            label_smoothing is not None
            and isinstance(label_smoothing, float)
            and (0 < label_smoothing < 1)
        ):
            self.loss = LabelSmoothedCrossEntropyLoss(
                self.classifier.targets, smoothing=label_smoothing
            ).to(self.device)
    ...
    def fit(
        ...
        **loader_kwargs,
        ...
        validation_log : Dataframe
                         Validation metrics after each epoch of training as a pandas dataframe
        """
        return super(StandardClassification, self).fit(
            training_dataset,
            epochs=epochs,
    ...
    def _make_dataloader(self, dataset, training=False, **loader_kwargs):
        ...
        loader_kwargs = self._dataloader_args(
            dataset, training=training, **loader_kwargs
        )

        if (
            training
            and loader_kwargs.get("sampler", None) is None
            and loader_kwargs.get("balance_method", None) is not None
        ):
            ...
            if not hasattr(dataset, "get_targets"):
                ...
                sampler = (
                    balanced_undersampling(dataset)
                    if method.lower() == "undersample"
...
def get_label_balance(dataset):
    ...
    train_weights = 1.0 / torch.tensor(counts, dtype=torch.float)
    sample_weights = train_weights[labels]
    class_freq = counts / counts.sum()
    if len(counts) < 10:
        tqdm.tqdm.write(
            "Class frequency: {}".format(" | ".join(f"{c:.2f}" for c in class_freq))
        )
    else:
        tqdm.tqdm.write(
            f"Class frequencies range from {class_freq.min():.2e} to {class_freq.max():.2e}"
        )
...
def balanced_undersampling(dataset, replacement=False):
    ...
    return WeightedRandomSampler(
        sample_weights, len(counts) * int(counts.min()), replacement=replacement
    )
...
def balanced_oversampling(dataset, replacement=True):
    ...
    sample_weights, counts = get_label_balance(dataset)
    return WeightedRandomSampler(
        sample_weights, len(counts) * int(counts.max()), replacement=replacement
    )
...
class LDAMLoss(torch.nn.Module):
    ...
    def _determine_drw_weights(self, beta=0.9999):
        ...
        return torch.from_numpy(
            per_cls_weights / np.sum(per_cls_weights) * len(self._cls_nums)
        ).float()
    ...
    def forward(self, x, target):
        ...
        batch_m = torch.matmul(
            self.m_list[None, :].to(index.device), index_float.transpose(0, 1)
        )
...
class BendingCollegeClassification(BendingCollegeWav2Vec, StandardClassification):
    def __init__(
        ...
        encoder_grad_frac=1.0,
        num_negatives=100,
        max_reconstruction_loss_frac=0.2,
        **kwargs,
    ):
        StandardClassification.__init__(
            self,
            bendr_model.classifier,
            metrics={
...
class DomainAwareProcess(StandardClassification):
    ...
    def fit(
        self,
        labeled_training,
        unlabeled_training_datasets=None,
        epochs=1,
        validation_dataset=None,
        resume_epoch=None,
        resume_iteration=None,
        ...
        **loader_kwargs,
        ...
        def _multi_domain_loaders(_domains):
            ...
            _multi_kwargs["batch_size"] = max(
                1, batch_size // len(_domains.get_domains())
            )
            return [
                self._make_dataloader(ds, training=True, **_multi_kwargs)
                for ds in _domains.get_domains()
            ]

        labeled_training = _multi_domain_loaders(labeled_training)
        unlabeled_training = (
            None
            if unlabeled_training_datasets is None
        ...
        resume_iteration = (
            1 if resume_iteration is None else resume_iteration % num_batches
        )
        ...
        train_log_interval = (
            num_batches if train_log_interval is None else train_log_interval
        )
        metrics = OrderedDict()
        ...
        for epoch in epoch_bar:
            ...
            unlabeled_iterator_list = (
                None
                if unlabeled_training is None
                else [cycle(ds) for ds in unlabeled_training]
            )
            for iteration in pbar:
                labeled_input, unlabelled_input = self._get_batch(
                    labelled_iterator_list, unlabeled_iterators=unlabeled_iterator_list
                )
                train_metrics = self.train_step(
                    *labeled_input, unlabeled=unlabelled_input
                )
                train_metrics["lr"] = self.optimizer.param_groups[0]["lr"]
                if "momentum" in self.optimizer.defaults:
                    train_metrics["momentum"] = self.optimizer.param_groups[0][
    ...
    def _get_batch(self, labeled_iterators, unlabeled_iterators=None):
        ...
        labeled = [
            super(DomainAwareProcess, self)._get_batch(it) for it in labeled_iterators
        ]
        if unlabeled_iterators is None:
            return labeled, None
        return labeled, [
            super(DomainAwareProcess, self)._get_batch(it) for it in unlabeled_iterators
        ]
