...
def rand_split(dataset, frac=0.75):
    ...
    samples = len(dataset)
    return random_split(
        dataset, lengths=[round(x) for x in [samples * frac, samples * (1 - frac)]]
    )
    ...
    try:
        if use_annotations:
            events = mne.events_from_annotations(
                raw, event_id=event_ids, chunk_duration=chunk_duration
            )[0]
        else:
            events = mne.find_events(raw)
            events = events[
                [i for i in range(len(events)) if events[i, -1] in event_ids.keys()], :
            ]
...
def skip_inds_from_bad_spans(epochs: mne.Epochs, bad_spans: list):
    ...
    skip_inds = list()
    for i, (start, end) in enumerate(zip(start_times, end_times, strict=False)):
