...
from dn3.utils import DN3ConfigException, make_epochs_from_raw, skip_inds_from_bad_spans
    ...
    def __init__(self, d: dict):
        ...
        for k in d:
            ...
            if isinstance(d[k], list):
                d[k] = [
                    _DumbNamespace(d[k][i]) if isinstance(d[k][i], dict) else d[k][i]
...
class ExperimentConfig:
    ...
    def __init__(self, config_filename: str, adopt_auxiliaries=True):
        ...
        """
        with open(config_filename) as fio:
        ...
        if "datasets" not in working_config.keys():
            raise DN3ConfigException(
                f"`datasets` not found in {[k.lower() for k in working_config.keys()]}"
            )
        ...
        ds_entries = (
            dict(zip(range(len(ds_entries)), ds_entries, strict=False))
            if isinstance(ds_entries, list)
            else ds_entries
        )
        ...
        if self.experiment is None:
            ...
            self._make_deep1010 = self.experiment.get(
                "deep1010", not self._make_common_chs
            )
        ...
        for i, name in enumerate(usable_datasets):
            if name in ds_entries.keys():
                self.datasets[name] = DatasetConfig(
                    name,
                    ds_entries[name],
                    deep1010=self._make_deep1010,
                    samples=self.global_samples,
                    sfreq=self.global_sfreq,
                    preload=preload,
                    return_trial_ids=return_trial_ids,
...
class DatasetConfig:
    ...
    def __init__(
        ...
        if self.filename_format is not None and not fnmatch(
            self.filename_format, "*{subject*}*"
        ):
        ...
        if self.picks is not None and not isinstance(self.picks, list):
            raise DN3ConfigException(
                f"Specifying picks must be done as a list. Not {self.picks}."
            )
        ...
        if self.events is not None:
            if not isinstance(self.events, (dict, list)):
                ...
                self.events = dict(
                    zip(self.events, range(len(self.events)), strict=False)
                )
        ...
        if not isinstance(self.rename_channels, dict):
            raise DN3ConfigException(
                "Renamed channels must map new values to old values."
            )
        ...
        if self.deep1010 is not None and (
            self.data_min is None or self.data_max is None
        ):
        ...
        if sfreq is not None and self.decimate > 1:
            print(
                f"{self.name}: No need to specify decimate ({self.decimate}) when sfreq is set ({sfreq})"
            )
            self.decimate = 1
        ...
        try:
            ...
            raise DN3ConfigException(f"Could not find required value: {e.args[0]}")
        if not self.toplevel.exists():
            raise DN3ConfigException(
                f"The toplevel {self.toplevel} for dataset {self.name} does not exists"
            )

        # The rest
        if adopt_auxiliaries and len(config) > 0:
            print(f"Adding additional configuration entries: {config.keys()}")
        ...
        if self.from_moabb is not None:
            try:
                self.from_moabb = MoabbDataset(
                    self.from_moabb.pop("name"),
                    self.toplevel.resolve(),
                    **self.from_moabb,
                )
            except KeyError:
                raise DN3ConfigException(
                    "MOABB configuration is incorrect. Make sure to use 'name' under MOABB to "
    ...
    def _is_narrowly_excluded(self, person_name, session_name):
        if person_name in self.exclude.keys():
            ...
            if (
                session_name in self.exclude[person_name].keys()
                and self.exclude[person_name][session_name] is None
            ):
                return True
        return False
    ...
    def _get_person_name(self, f: Path):
        if self.filename_format is None:
            ...
            person = search(
                self.filename_format, str(f).replace(f"{self.toplevel}/", "")
            )
            if person is None:
    ...
    def _get_session_name(self, f: Path):
        ...
        if self.filename_format is not None:
            if fnmatch(self.filename_format, "*{session*}*"):
                ...
                sess_name = dict(search(self.filename_format, str(f))).get(
                    "session", search(self.filename_format, str(f))["session"]
                )
        return sess_name
    ...
    def _add_deep1010(self, ch_names: list, deep1010map: np.ndarray, unused):
        for i, (old_names, old_map, unused, count) in enumerate(
    ...
    def _load_raw(self, path: Path):
        ...
        if path.suffix in self._extension_handlers:
            return self._extension_handlers[path.suffix](
                str(path), preload=self.preload
            )
        print(f"Handler for file {path!s} with extension {path.suffix} not found.")
        ...
        raise DN3ConfigException(f"No supported/provided loader found for {path!s}")
    ...
    def _prepare_session(
        ...
        if lowpass is not None and (new_sfreq < 2 * lowpass) and new_sfreq != raw_sfreq:
            raise DN3ConfigException(
                "Could not create raw for {}. With lowpass filter {}, sampling frequency {} and "
                "new sfreq {}. This is going to have bad aliasing!".format(
                    raw.filenames[0], raw.info["lowpass"], raw.info["sfreq"], new_sfreq
                )
            )
        ...
        # Pick types
        picks = (
            pick_types(raw.info, **{t: t in picks for t in DatasetConfig._PICK_TYPES})
            if DatasetConfig._picks_as_types(picks)
        ...
        picks = [
            ...
            for idx in picks
            if True
            not in [fnmatch(raw.ch_names[idx], pattern) for pattern in exclude_channels]
        ]
        ...
        for new_ch, pattern in rename_channels.items():
            for old_ch in [
                raw.ch_names[idx]
                for idx in picks
                if fnmatch(raw.ch_names[idx], pattern)
            ]:
    ...
    def _construct_session_from_config(self, session, sess_id, thinker_id):
        ...
        if thinker_id in self.exclude.keys():
            if sess_id in self.exclude[thinker_id].keys():
                ...
                if bad_spans is None:
                    raise DN3ConfigException(f"Skipping {thinker_id} - {sess_id}")
        ...
        if self._create_raw_recordings:
            if self._on_the_fly:
                recording = RawOnTheFlyRecording(
                    raw,
                    tlen,
                    lambda s: load_and_prepare(s)[1],
                    stride=self.stride,
            ...
            use_annotations = self.events is not None and True in [
                isinstance(x, str) for x in self.events.keys()
            ]
            if not isinstance(
                raw, (mne.Epochs, mne.epochs.EpochsFIF)
            ):  # Annoying other epochs type
                if use_annotations and self.annotation_format is not None:
                    patt = self.annotation_format.format(
                        subject=thinker_id, session=sess_id
                    )
                    ann = [str(f) for f in session.parent.glob(patt)]
                    if len(ann) > 0:
                        if len(ann) > 1:
                            print(
                                f"More than one annotation found for {patt}. Falling back to {ann[0]}"
                            )
                        raw.set_annotations(read_annotations(ann[0]))
                epochs = make_epochs_from_raw(
                    raw,
                    self.tmin,
                    tlen,
                ...
                epochs = raw
            event_map = (
                {v: v for v in self.events.values()} if use_annotations else self.events
            )

            self._unique_events = self._unique_events.union(
                set(np.unique(epochs.events[:, -1]))
            )
            recording = EpochTorchRecording(
                epochs,
                ch_ind_picks=picks,
        ...
        if len(recording) == 0:
            raise DN3ConfigException(
                f"The recording at {session!s} has no viable training data with the configuration options "
        ...
        if self.deep1010 is not None:
            ...
            _dum = _DumbNamespace(
                dict(
                    ...
                    info=dict(data_max=self.data_max, data_min=self.data_min),
                )
            )
        ...
        if recording.sfreq != new_sfreq:
            new_sequence_len = (
                int(tlen * new_sfreq) if self._samples is None else self._samples
            )
            recording.add_transform(
                TemporalInterpolation(new_sequence_len, new_sfreq=new_sfreq)
            )
    ...
    def _construct_thinker_from_config(self, thinker: list, thinker_id):
        ...
        if self._custom_thinker_loader is not None:
            ...
            for sess in thinker:
                ...
                try:
                    new_session = self._construct_session_from_config(
                        sess, sess_name, thinker_id
                    )
                    after_cb = (
                        None
                        if self._session_callback is None
                        else self._session_callback(new_session)
                    )
        ...
        if self._sfreq is not None and thinker.sfreq != self._sfreq:
            new_sequence_len = (
                int(thinker.sequence_length * self._sfreq / thinker.sfreq)
                if self._samples is None
                else self._samples
            )
            thinker.add_transform(
                TemporalInterpolation(new_sequence_len, new_sfreq=self._sfreq)
            )
    ...
    def auto_construct_dataset(self, mapping=None, **dsargs):
        ...
        if self.dumped is not None:
            ...
            if path.exists():
                ...
                tqdm.tqdm.write(
                    "Could not load pre-dumped data, falling back to original data..."
                )
        ...
        for t in tqdm.tqdm(mapping, desc=description, unit="person"):
            try:
                ...
                after_cb = (
                    None
                    if self._thinker_callback is None
                    else self._thinker_callback(new_thinker)
                )
                thinkers[t] = new_thinker if after_cb is None else after_cb
            except DN3ConfigException:
                tqdm.tqdm.write(
                    f"None of the sessions for {t} were usable. Skipping..."
                )
...
class RawOnTheFlyRecording(RawTorchRecording):
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__(
            raw, tlen, session_id, person_id, stride, ch_ind_picks, decimate, **kwargs
        )
    ...
    def preprocess(self, preprocessor, apply_transform=True):
        ...
        if result is not None:
            raise DN3ConfigException(
                "Modifying raw after preprocessing is not supported with on-the-fly"
            )
        if apply_transform:
