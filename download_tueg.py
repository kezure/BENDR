...
class Dataset:
    ...
    def session_gen(self, mode="sequential"):
        ...
        for file in os.scandir(self.path):
            if re.match(r"v\d+.\d+.\d+", file.name):
    ...
    def download(
        self, url, username=None, password=None, maxSize=float("Inf"), currPath=None
    ):
        ...
        nodes = [n for n in soup.find_all("a") if n.text not in ignoreTheseAttrs]
        ...
        for fileName in fileNames:
            ...
            if size > maxSize:
                print(
                    f"Bytes limit would be breached. Stopping before downloading {size}-bytes file {fURL}."
                )
                return False, maxSize
            else:
                if not maxSize == float("inf"):
                    print(f"{maxSize} bytes to go. ", end="")
                print("Downloading " + fURL + ".")
                r = requests.get(
                    fURL, auth=(username, password), allow_redirects=True, stream=True
                )
                size = len(r.content)
...
class Session:
    ...
    def __init__(self, path=None):
        ...
        if len(p.parts[-1].split("_")) != 4:
            print(path)
            raise ValueError(
                "Path does not end with a valid session ID. " + p.parts[-1]
            )
        self.path = path
...
def getDir(dir, f_types=None):
    ...
    if f_types is None:
        ...
        raise TypeError(
            "f_types should be a list e.g. f_types=['txt'] or f_types=['txt','edf']"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path",
        default="./data/tueg/tuh_eeg_abnormal",
        help="The path to save the dataset to.",
    )
    parser.add_argument(
        "--url",
        default="https://www.isip.piconepress.com/projects/tuh_eeg/downloads/tuh_eeg_abnormal/",
        help="The web address for the top directory of the data to be downloaded.",
    )
    ...
    ds.download(
        args.url, username=args.username, password=args.password, maxSize=10**11
    )
