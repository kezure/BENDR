...
class RandomTemporalCrop(BatchTransform):
    ...
    def __call__(self, x, training=False):
        ...
        crop_len = np.random.randint(
            int((1 - self.max_crop_frac) * trial_len), trial_len
        )
...
class RandomTemporalEndCrop(BatchTransform):
    ...
    def __call__(self, x, training=False):
        ...
        if self.crop_weights is None:
            ...
            self.crop_weights = np.ones(
                int(x.shape[self.temporal_axis] * self.end_crop_frac)
            )
        ...
        crop_location = np.random.choice(
            inds, p=self.crop_weights / self.crop_weights.sum()
        )
