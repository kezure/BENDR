...
class TemporalPadding(InstanceTransform):
    ...
    def __call__(self, x):
        pad = [self.start_padding, self.end_padding] + [
            0 for _ in range(2, x.shape[-1])
        ]
        return torch.nn.functional.pad(
            x, pad, mode=self.mode, value=self.constant_value
        )
...
class TemporalInterpolation(InstanceTransform):
    ...
    def __call__(self, x):
        ...
        if len(x.shape) == 2:
            return interpolate(
                x.unsqueeze(0), self._new_sequence_length, mode=self.mode
            ).squeeze(0)
...
class TemporalCrop(InstanceTransform):
    ...
    def _get_start_offset(self, full_length):
        ...
        if self.start_offset is None:
            start_offset = (
                np.random.choice(possible_starts) if possible_starts > 0 else 0
            )
        elif isinstance(self.start_offset, list):
            start_offset = self.start_offset + [
                0 for _ in range(len(self.start_offset), possible_starts)
            ]
...
class CropAndResample(TemporalInterpolation):
    def __init__(
        ...
        if crop_side not in ["right", "left", "both"]:
            raise ValueError(
                "The crop-side should either be 'left', 'right', or 'both'"
            )
    ...
    def __call__(self, x):
        ...
        if self.crop_side == "right":
            ...
            offset = self.trunc_norm(
                x.shape[-1] - self._new_sequence_length, self.stdev, max_diff
            )
            return super(CropAndResample, self).__call__(x[:, offset:])
    ...
    def new_channels(self, old_channels: np.ndarray):
        ...
        for row in range(self.mapping.shape[1]):
            ...
            if len(active) > 0:
                ...
                types.append(None)
        return np.array(list(zip(channels, types, strict=False)))
...
class MappingDeep1010(InstanceTransform):
    ...
    def __init__(self, dataset, max_scale=None, return_mask=False):
        ...
        if max_scale is not None:
            ...
            or dataset.info.data_max is None
            or dataset.info.data_min is None
        ):
    ...
    def __call__(self, x):
        if self.max_scale is not None:
            scale = 2 * (
                torch.clamp_max((x.max() - x.min()) / self.max_scale, 1.0) - 0.5
            )
        else:
            scale = 0
    ...
    def new_channels(self, old_channels: np.ndarray):
        ...
        for row in range(self.mapping.shape[1]):
            ...
            if len(active) > 0:
                ...
                channels.append(None)
        return np.array(list(zip(channels, DEEP_1010_CH_TYPES, strict=False)))
...
class To1020(InstanceTransform):
    ...
    def __init__(
        ...
        if include_ref_chs:
            self._inds_20_div.append(
                [DEEP_1010_CHS_LISTING.index(ch) for ch in ["A1", "A2"]]
            )
        if include_scale_ch:
...
class AdditiveEogDeep1010(InstanceTransform):
    ...
    def __call__(self, x):
        ...
        for which_eog in EOG_INDS:
            this_affected_ids = affected_inds[
                torch.rand_like(affected_inds.float()).lt(0.25)
            ]
            x[this_affected_ids, :] = (
                self.max_intensity
                * torch.rand_like(this_affected_ids.float()).unsqueeze(-1)
                * x[which_eog]
            )
            # Short circuit the random call as these are more frequent
            if self.blanking_p != 0 and (
                self.blanking_p == 1 or torch.rand(1) < self.blanking_p
            ):
                x[which_eog, :] = 0
        return x
...
class UniformTransformSelection(InstanceTransform):
    def __init__(self, transform_list, weights=None, suppress_warnings=False):
        ...
        if weights is None:
            self._choice_weights = [
                1 / len(transform_list) for _ in range(len(transform_list))
            ]
        else:
    ...
    def new_channels(self, old_channels):
        all_new = [
            transform.new_channels(old_channels) for transform in self.transforms
        ]
