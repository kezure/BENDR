...
class TVector(Classifier):
    def __init__(
        self,
        targets=None,
        channels=len(DEEP_1010_CHS_LISTING),
        samples=None,
        hidden_size=384,
        dropout=0.1,
        ignored_inds=(SCALE_IND,),
        incoming_channels=None,
        norm_groups=16,
        return_tvectors=False,
    ):
        self.hidden_size = hidden_size
        self.num_target_people = targets
        self.dropout = dropout
        super(TVector, self).__init__(
            targets, None, channels, return_features=return_tvectors
        )
        self.ignored_ids = ignored_inds
        self.mapping = (
            None
            if incoming_channels is None
            else map_named_channels_deep_1010(incoming_channels)
        )
...
class ClassificationWithTVectors(StandardClassification):
    def __init__(
        ...
        tvector_model: TVector,
        loss_fn=None,
        cuda=False,
        metrics=None,
        learning_rate=None,
    ):
        super(ClassificationWithTVectors, self).__init__(
            classifier=classifier,
            tvector_model=tvector_model,
            loss_fn=loss_fn,
            cuda=cuda,
            metrics=metrics,
    ...
    def build_network(self, **kwargs):
        ...
        self.attn_tvect = torch.nn.Parameter(
            torch.ones(
                (
                    self.tvector_model.num_features_for_classification,
    ...
    def forward(self, *inputs):
        ...
        added = (
            classifier_features.view(batch_size, -1)
            + t_vectors.view(batch_size, -1) @ self.attn_tvect
        )
...
class TVectorConcatenation(InstanceTransform):
    ...
    def new_channels(self, old_channels):
        return old_channels + [
            f"T-vectors-{i+1}"
            for i in range(self.tvect.num_features_for_classification)
        ]
